package org.nanick.cellularconnectioninfo;
...
public class CellularConnectionInfo {
    ...
    public Bands[] bands;
    public CellularConnectionInfo(Context context, TextView[] results, Bands[] bands) {
        this.bands = bands;
        ...
        try {
            if (this.csv.createNewFile()) {
                ...
                Log.i("CellularConnectionInfo", "File created.");
            } else {
                Log.e("CellularConnectionInfo", "Failed to create file.");
            }
        }
        catch (IOException e) {
    ...
    public void start() {
        ...
        if (!gpsEnabled && !networkEnabled) {
            Log.e("CellularConnectionInfo", "No location providers enabled.");
        }
    }
    public void stop() {
    ...
    private class LocationListenerImpl implements LocationListener {
        ...
        public void onLocationChanged(Location location) {
            if (location != null) {
                ...
                Log.e("CellularConnectionInfo", "Location is null.");
            }
        }
        @Override
    ...
    public class SpeedTestTask extends AsyncTask<Void, Void, Double> {
        ...
        public SpeedTestTask(CellInfoObj cellInfoObj, TextView[] tv){
            ...
            this.textViews = tv;
            /*Log.i("CellularConnectionInfo",this.textViews.length+" textviews");
            for(int i = 0; i < this.textViews.length; i++){
                TextView tvn = this.textViews[i];
                String tvn_name = tvn.getResources().getResourceName(tvn.getId());
                Log.i("CellularConnectionInfo","id: "+tvn_name+", index: "+i);
            }*/
        ...
        protected void onPostExecute(Double mbps) {
            if (mbps != null) {
                ...
                this.cio.DownloadSpeed = Double.parseDouble(new DecimalFormat("0.00").format(mbps));
                Log.i("CellularConnectionInfo",new DecimalFormat("0.00").format(mbps)+" Mbps download");
                try {
                    Log.i("CellularConnectionInfo",JSONConvector.toJSON(this.cio));
    ...
    private void getCellularDetails() {
        ...
        if(location != null){
            ...
            if (cellInfo instanceof CellInfoLte) {
                ...
                //IntStream.of((((CellInfoLte) cellInfo).getCellIdentity()).getBands()).forEach(x -> sj.add(String.valueOf(x)));

                //IntStream.of(cellId.getBands()).forEach(x -> sj.add(String.valueOf(x)));
                //Log.i("CellularConnectionInfo", "bands: " + sj.toString());
                //cio.Cqi = sj.toString();
                ...
                if(!found){
                    Log.i("CellularConnectionInfo","Could not find band from EARFCN!");
                } else {
                    Log.i("CellularConnectionInfo","Band: "+cio.Band.band+", spectrum: "+cio.Band.name);
                }
